...
<img src="${parkImage}" class="rounded mx-auto d-block parkDetailImage" id="detailImage" />
	...
	<c:forEach items="${weather}" var="dayWeather">
		<c:if test="${dayWeather.fiveDayForcastValue == 1}">
			<div class="todaysWeather  mx-auto">
				<h4 class="text-center">Today's Forecast</h4>
		</c:if>
		<c:if test="${dayWeather.fiveDayForcastValue != 1}">
...
<form:form action="${parkDetail}" mathod="POST">
	...
	<button type="submit" name="temp" value="farenheit"
		class="btn btn-primary">Farenheit</button>
	<button type="submit" name="temp" value="celcius"
		class="btn btn-secondary">Celcius</button>
	</div>
